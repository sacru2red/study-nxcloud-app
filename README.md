...
**Q. 자동 테스트로 무엇을 검증했나요?**  
A. `apps/backend-e2e`: tenant 격리, 업로드·인덱싱·채팅·「문서에서 확인 불가」·Admin usage·quota 등 11건. `apps/frontend-e2e`: 로그인·Admin 접근 제어·데모 캡처. 체크리스트 하단에 최근 통과 기록이 있습니다.
...
A. 파일 저장 WebDAV, 사용량 OCS API, Admin 화면(스크린샷 7)·`GET /api/admin/tenants/:tenantId/users-usage`(`lastCollectedAt` 포함). Docker·seed·init 스크립트는 `infra/`, `prisma/seed.ts`에 있습니다.
...
A. 스크린샷 5-1·5-2: 질문 답변 + source 카드(파일명·페이지·문단). 스크린샷 6: 문서 밖 질문 → 「문서에서 확인 불가」·sources 빈 배열. 백엔드 E2E Test 7이 동일 동작을 assert합니다.
...
**Q. 체크리스트에 [ ]로 남은 항목은?**  
A. **bbox**: DB 필드만 있고 좌표 추출·UI 하이라이트 없음. **Q&A 10초**: 성능 목표이며, 환경·모델에 따라 달라 [벤치마크](./docs/development.md#rag-응답-시간-벤치마크)로 측정하도록 안내했을 뿐, 항상 보장하지 않습니다.
